...
data_full_recoverable = data_full_filter.drop(index=idxs).reset_index(drop=True)
...
data_processed_recoverable = data_processed.drop(index=idxs).reset_index(drop=True)
data_processed_recoverable.to_csv('data_preprocessed_recoverable.csv', index=False)
# for i in range(n_params):
#     for j in range(i+1):
#         plt.figure(figsize=(4, 4))
